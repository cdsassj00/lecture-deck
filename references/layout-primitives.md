...
- 좌측 텍스트 칼럼 중단이 비어도 새 문장을 만들지 않는다. 원문에 실제 인용·핵심 문장이 있을 때만 `.pull`을 쓰고, 그렇지 않으면 폰트 크기·행간·여백·표/도식 배치로 채운다.
- 하단은 `.note`(정리/출처 바)로 마감해 바닥 여백을 없앤다.
- 박스 안 텍스트가 적으면 해당 박스의 제목·본문 폰트 크기를 키워도 된다. 이때 새 멘트를 추가하지 말고 원문 텍스트를 크게 배치한다.
...
| 인용 | `.pull`(+ span) | 원문에 실제로 있는 핵심 문장/원칙. 빈 공간을 채우기 위한 새 문장 작성 금지 |
